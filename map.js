...
function legend() {
	...
	var head = document.createElement("p");
        ...
        element.appendChild(head);
	 var img = new Image();
	  img.src = "http://localhost:8888/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" +layer.options.layers;
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:8888/geoserver/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:8888/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
                    ...
                    $(xml).find('xsd\\:sequence').each(function() {
                        $(this).find('xsd\\:element').each(function() {
                            ...
                            //alert(type);
                            if (value == 'Jenis_Fasilitas_Kesehatan') {
                                select.append("<option class='ddindent' value=" + i + ">" + value + "</option>");
								i=i+1;
                            }
                            // if (value != 'geom' && value != 'the_geom' && value != 'ATM_geom' && value != 'Alamat' && value != 'Fasilitas') {
                            //     select.append("<option class='ddindent' value=" + i + ">" + value + "</option>");
							// 	i=i+1;
                            // }
...
$(function () {
    $("#attributes").change(function () {
        ...
        var value_layer = $('#layer option:selected').val();
        operator.options[0] = new Option('Select All', "");
        //alert(value_attribute);
        //alert(value_layer);
        $(document).ready(function () {
            $.ajax({
                type: "GET",
                url: "http://localhost:8888/geoserver/wfs?service=WFS&request=GetFeature&version=1.1.0&typeName=" + value_layer + "&outputFormat=application/json",
                dataType: "json",
                success: function (data) {
                    ...
                    for (var i = 0; i < data.features.length; i++) {
                        //show unique data in col[1] add to dropdown option
                        if (uniqueValues.indexOf(data.features[i].properties[value_attribute]) === -1) {
                            uniqueValues.push(data.features[i].properties[value_attribute]);
                            select.append("<option class='ddindent' value='" + data.features[i].properties[value_attribute] + "'>" + data.features[i].properties[value_attribute] + "</option>");
...
function query() {
    ...
    var value_bpjs = bpjs.value;
    console.log(value_bpjs)
    //alert("halo");

    var non_tunai = document.getElementById("non_tunai");
    var value_non_tunai = non_tunai.value;

    var darurat = document.getElementById("darurat");
    var value_darurat = darurat.value;

    var ambulan = document.getElementById("ambulan");
    var value_ambulan = ambulan.value;

    var rawat_inap = document.getElementById("rawat_inap");
    var value_rawat_inap = rawat_inap.value;

    var apotek = document.getElementById("apotek");
    var value_apotek = apotek.value;



    var url = "http://localhost:8888/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer
    if(value_operator != ""){
        url += "&CQL_FILTER=" + value_attribute + "%20=%20" + "'" + value_operator+"'"
    }
    else {
        var filterParts = [];

        if (bpjs.checked) {
            filterParts.push("Layanan_BPJS = '" + value_bpjs + "'");
        }
        if (non_tunai.checked) {
            filterParts.push("Pembayaran_Non_Tunai = '" + value_non_tunai + "'");
        }
        if (darurat.checked) {
            filterParts.push("Layanan_Darurat = '" + value_darurat + "'");
        }
        if (ambulan.checked) {
            filterParts.push("Layanan_Ambulan = '" + value_ambulan + "'");
        }
        if (rawat_inap.checked) {
            filterParts.push("Layanan_Rawat_Inap = '" + value_rawat_inap + "'");
        }
        if (apotek.checked) {
            filterParts.push("Apotek = '" + value_apotek + "'");
        }
    
        if (filterParts.length > 0) {
            url += "&CQL_FILTER=" + filterParts.join(" AND ");
        }
    }
    if (bpjs.checked==true){
        url += "AND Layanan_BPJS" +"%20=%20" + "'" + value_bpjs+ "'" 
    }
    if (non_tunai.checked==true){
        url += "AND Pembayaran_Non_Tunai" +"%20=%20" + "'" + value_non_tunai+ "'"
    }
    if (darurat.checked==true){
        url += "AND Layanan_Darurat" +"%20=%20" + "'" + value_darurat+ "'"
    }
    if (ambulan.checked==true){
        url += "AND Layanan_Ambulan" +"%20=%20" + "'" + value_ambulan+ "'"
    }
    if (rawat_inap.checked==true){
        url += "AND Layanan_Rawat_Inap" +"%20=%20" + "'" + value_rawat_inap+ "'"
    }
    if (apotek.checked==true){
        url += "AND Apotek" +"%20=%20" + "'" + value_apotek+ "'"
    }
...
caption.style.color = 'black'; // Ganti 'black' dengan warna yang Anda inginkan
caption.style.fontWeight = 'bold';
caption.innerHTML = " Data Yang Ditemukan : "+data.features.length+" ";
if (data.features.length < 1){
    caption.innerHTML = " Data Tidak Ditemukan "
}
...
function wms_layers() {
  ...
  $("#wms_layers_window").modal('show');
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:8888/geoserver/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
...
function add_layer() {
    ...
    //alert(layer_name);
    var layer_wms = L.tileLayer.wms('http://localhost:8888/geoserver/wms?', {
        layers: layer_name,
        transparent: 'true',
        format: 'image/png'
...
function getinfo(e) {
	...
	overlays.eachLayer(function (layer) {
	   var url = 'http://localhost:8888/geoserver/wms?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetFeatureInfo&FORMAT=image%2Fpng&TRANSPARENT=true&QUERY_LAYERS=' + layer.options.layers + '&LAYERS=' + layer.options.layers + '&INFO_FORMAT=text%2Fhtml&X=' + x + '&Y=' + y + '&CRS=EPSG%3A4326&STYLES=&WIDTH=' + width + '&HEIGHT=' + height + '&BBOX=' + bbox;
console.log(url);   
